...
* // returns 2
*/
function dlastIndexOfRow( order, M, N, A, LDA, x, strideX, workspace, strideW ) { // eslint-disable-line max-len
	var sa1;
	var sa2;
	var s;
	var l;
	...
	if ( l === null ) {
		throw new TypeError( format( '2f7Fx', order ) );
	}
	if ( isRowMajor( l ) ) {
		s = N;
		sa1 = LDA;
	...
	if ( LDA < max( 1, s ) ) {
		throw new RangeError( format( '2f7IR', max( 1, s ), LDA ) );
	}
